...
export function useProcessStatus(
  ...
  const [status, setStatus] = useState<'none' | 'indexing' | 'indexed'>(
    initialStatus,
  );
  ...
  const lastKnowledgeBaseIdRef = useRef<string | null | undefined>(
    knowledgeBaseId,
  );
  ...
  useEffect(() => {
    ...
    if (!processId || processIdChanged || knowledgeBaseIdChanged) {
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `Updating status to: ${currentInitialStatus} (processId: ${processId}, knowledgeBaseId: ${knowledgeBaseId})`,
        );
      }
  ...
  async function checkProcessStatus() {
    ...
    if (
      ...
      now - lastPollTimeRef.current < 5000 ||
      !processId
    ) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.warn(
            `Failed to fetch process status (attempt ${errorCountRef.current}):`,
            response.statusText,
          );
        }
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          `Mapping API status "${processStatus}" to component status "${newStatus}"`,
        );
      }
      ...
      if (session?.user?.selectedSpace) {
        try {
          const indexedPayload =
            indexedVersions.length > 0 ? { indexedVersions } : {};

...
const humanize = (slug: string) =>
  slug
    ...
    .join(' ');
export function DocPromptsPanel({
  knowledgeBaseId,
  className,
}: DocPromptsPanelProps) {
  const { data, error, isLoading, mutate } = useSWR<DocPromptsResponse>(
    knowledgeBaseId
      ? `/api/docs/prompts?knowledgeBaseId=${encodeURIComponent(knowledgeBaseId)}`
      : null,
    fetcher,
  );
  ...
  const toggleEditor = (id: string, open: boolean) => {
    setDrafts((prev) =>
      prev.map((entry) =>
        entry.id === id ? { ...entry, isEditing: open } : entry,
      ),
    );
  };
  ...
  const handleTitleChange = (id: string, title: string) => {
    ...
    setDrafts((prev) =>
      prev.map((entry) =>
        entry.id === id
          ? { ...entry, title, slug: normalized || entry.slug }
          : entry,
      ),
    );
  };
  ...
  const handlePromptChange = (id: string, prompt: string) => {
    setDrafts((prev) =>
      prev.map((entry) => (entry.id === id ? { ...entry, prompt } : entry)),
    );
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          body?.detail || body?.error || 'Unable to save prompts',
        );
      }
      ...
      toast.error(
        err instanceof Error ? err.message : 'Unable to save prompts',
      );
    } finally {
      setIsSaving(false);
    }
  ...
  if (!knowledgeBaseId) {
    return (
      <div
        className={cn(
          'rounded-2xl border bg-card/60 p-4 shadow-sm text-sm text-muted-foreground',
          className,
        )}
      >
    ...
    <section className={cn('space-y-4', className)}>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Tell the agent what to write each sync.
          </p>
          {updatedLabel && (
      ...
      {isLoading ? (
        ...
        <div className="rounded-md border border-dashed px-3 py-6 text-sm text-muted-foreground text-center">
          No docs selected. Use the arrow to pick a suggestion or add your own
          instructions.
        </div>
      ) : (
        <div className="space-y-2">
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex gap-1">
                  <Button
                    ...
                    className="h-7 w-7"
                    onClick={() =>
                      toggleEditor(entry.id, !(entry.isEditing ?? false))
                    }
                  >
              ...
              {entry.isEditing ? (
                <div className="mt-3 space-y-3">
                  <Input
                    value={entry.title}
                    onChange={(event) =>
                      handleTitleChange(entry.id, event.target.value)
                    }
                  ...
                  <Textarea
                    value={entry.prompt}
                    onChange={(event) =>
                      handlePromptChange(entry.id, event.target.value)
                    }
                  ...
                  <div className="flex justify-end">
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      onClick={() => toggleEditor(entry.id, false)}
                    >
                      Done
                    </Button>
                  </div>
          ...
          >
            ...
            {entries.map(([slug, meta]) => (
              <DropdownMenuItem
                ...
                onSelect={() =>
                  onInsertSuggestion(slug, meta.prompt, meta.title)
                }
              >

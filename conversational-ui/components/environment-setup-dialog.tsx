...
export function EnvironmentSetupDialog({
  ...
  const canSubmit =
    !!knowledgeBaseId &&
    (globalSetup.trim().length > 0 || projectSetup.trim().length > 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const detail =
          (data && (data.detail || data.error)) || 'Failed to save environment';
        toast.error(detail);
        return;
      }
    ...
    const fetchLatest = async () => {
      ...
      try {
        const res = await fetch(
          `/api/repo/environments?knowledgeBaseId=${knowledgeBaseId}`,
        );
        if (res.ok) {
          const data = await res.json();
          // Prefill on open, but do not overwrite if user already typed
          if (typeof data.global === 'string' && !dirtyGlobal)
            setGlobalSetup(data.global || '');
          if (typeof data.project === 'string' && !dirtyProject)
            setProjectSetup(data.project || '');
        }
      } catch {
        // ignore
      } finally {
      ...
      <AlertDialogContent className="sm:max-w-[560px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="text-xs text-muted-foreground space-y-1">
            ...
            <p>
              Use <strong>Global setup</strong> to install system tools and
              dependencies (apt-get, curl). Do not clone the repository here; it
              will be cloned automatically using your URL/token.
            </p>
            <p>
              <strong>Project setup</strong> runs from the cloned repository
              root to install project dependencies (e.g., <code>uv sync</code>,{' '}
              <code>pnpm install</code>).
            </p>
            <p>
              You can fill either or both. Executed before the remote coding
              agent starts.
            </p>
          </div>

...
async function getRepoUrlFromBackend(
  knowledgeBaseId: string,
): Promise<string | null> {
  try {
    const response = await fetch(
      `${process.env.CUDU_ENDPOINT}/processes?knowledge_base_id=${knowledgeBaseId}`,
    );

    if (!response.ok) {
      console.warn(
        `Failed to fetch repo info for knowledge base ${knowledgeBaseId}: ${response.statusText}`,
      );
      return null;
    }
    ...
    const repoUrl =
      data.repo_url || data.repository_url || data.git_url || data.url;
    ...
    if (
      ...
      if (process.events && Array.isArray(process.events)) {
        const repoConnectedEvent = process.events.find(
          (event: any) => event.type === 'repository_connected',
        );
        if (repoConnectedEvent?.url) {
    ...
    console.error(
      `Error fetching repo info for knowledge base ${knowledgeBaseId}:`,
      error,
    );
    return null;
  }
...
async function backfillConnectedRepoUrls() {
  ...
  const spacesNeedingBackfill = await db
    ...
    .from(space)
    .where(
      and(isNotNull(space.knowledgeBaseId), isNull(space.connectedRepoUrl)),
    );

  console.info(
    `Found ${spacesNeedingBackfill.length} spaces needing repo URL backfill`,
  );
  ...
  for (const spaceRecord of spacesNeedingBackfill) {
    ...
    try {
      console.info(
        `Processing space "${spaceRecord.name}" (${spaceRecord.id})`,
      );
      ...
      if (!repoUrl) {
        console.warn(
          `No repo URL found for space "${spaceRecord.name}" with knowledge base ${spaceRecord.knowledgeBaseId}`,
        );
        spacesSkipped++;
        continue;
      }
      ...
      if (updateResult.length > 0) {
        console.info(
          `Updated space "${spaceRecord.name}" with repo URL: ${repoUrl}`,
        );
        spacesUpdated++;
      } else {
      ...
      console.error(
        `Error processing space "${spaceRecord.name}":`,
        errorMessage,
      );
      errors.push({
  ...
  console.info(
    `Backfill completed: ${spacesProcessed} spaces processed, ${spacesUpdated} updated, ${spacesSkipped} skipped, ${errors.length} errors`,
  );

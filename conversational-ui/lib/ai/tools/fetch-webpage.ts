...
export const fetchWebpage = ({ dataStream }: FetchWebpageProps) =>
  tool({
    description:
      'Fetch and extract readable content from a webpage URL. Handles both static and dynamic websites.',
      ...
      try {
        ...
        const getChromiumExecutablePath = () => {
          ...
          try {
            ...
            const chromiumDirs = readdirSync(browsersPath).filter((dir) =>
        ...
        console.log(
          `Successfully fetched webpage: ${url}, ${result.wordCount} words, took ${duration}ms`,
        );

...
export async function GET(request: Request) {
  try {
    ...
    if (cached) {
      console.log(
        `Using cached process snapshot for repository details: ${processId}`,
      );
      data = cached;
    } else {
    ...
    const repoEvent = data.events?.find(
      (event: { type: string }) =>
    ...
    const lastSuccessTime = latestRepositoryIndexedEvent
      ...
      : 0;
    const failureWithoutSuccess =
      connectionFailedEvent && !latestRepositoryIndexedEvent;
    const failureAfterSuccess =
      connectionFailedEvent &&
      latestRepositoryIndexedEvent &&
      failedEventTime > lastSuccessTime;

    // If we have a failure and no success yet, keep the hard error flow
    if (failureWithoutSuccess) {
      console.log(
        `Repository connection failed: ${connectionFailedEvent.error_message}`,
      );
      return NextResponse.json({
        connected: false,
        error: true,
    ...
    const indexingStartTime =
      latestIndexationRequestedEvent?.occurred_at || repoEvent.occurred_at;

    // Get the most recent token permissions (from either initial connection or token rotation)
    const tokenRotatedEvents = data.events.filter(
      (e: any) => e.type === 'repository_token_rotated',
    );
    ...
    if (tokenRotatedEvents.length > 0) {
      ...
      const mostRecentTokenRotation = tokenRotatedEvents.reduce(
      ...
      if (
        new Date(mostRecentTokenRotation.occurred_at) >
        new Date(repoEvent.occurred_at)
      ) {
        mostRecentTokenPermissions =
          mostRecentTokenRotation.token_permissions ||
          mostRecentTokenPermissions;
      }
    }
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        case 403:
          ...
          errorMessage =
            'Permission denied. Please check your access rights to this repository.';
          break;
        case 404:
    ...
    return NextResponse.json(
      {
        ...
        errorType: 'unexpected_error',
        errorMessage:
          'An unexpected error occurred while trying to connect to the repository.',
      },
      { status: 500 },
    );
  }

...
export async function GET(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        `Error fetching process status: ${response.statusText}`,
        errorData,
      );
    ...
    if (
      ...
      if (latestEvent.type) {
        ...
        console.log(
          `Derived status from event type: ${enhancedResponse.status}`,
        );
      }
    }

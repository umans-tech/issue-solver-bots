...
> [!IMPORTANT] Ensure that your `CODING_AGENT_ACCESS_TOKEN` has the necessary permissions to commit and push
...
> [!CAUTION] If you are using self-managed runners, ensure they are configured with Docker-in-Docker support as
...
> [!IMPORTANT] Ensure that the `CODING_AGENT_ACCESS_TOKEN` has permissions to access the issue if using
...
> [!CAUTION] Modifying Docker settings may require changes to your runner configuration.

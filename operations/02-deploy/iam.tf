...
# IAM policy to allow webapi Lambda to access the knowledge blob S3 bucket
resource "aws_iam_policy" "webapi_blob_s3_access" {
  name        = "webapi-blob-s3-access${local.environment_name_suffix}"
  description = "Allow webapi Lambda to read/write in knowledge blob S3 bucket"
...
# Attach S3 access policy to webapi execution role
resource "aws_iam_role_policy_attachment" "webapi_blob_s3_access" {
  role       = aws_iam_role.webapi_lambda_exec.name
  policy_arn = aws_iam_policy.webapi_blob_s3_access.arn
}
